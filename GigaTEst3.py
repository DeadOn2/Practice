...
class StudentModel(nn.Module):
    def __init__(self, vocab_size, speaker_dim=Config.encoder_dim):
        ...
        self.mel_prenet = nn.Sequential(
            nn.Linear(Config.n_mels, self.d_model),
            nn.ReLU()
        )
    ...
    def forward(self, text, speaker_emb, mel_target=None):
        ...
        memory = self.encoder(x)  # [Batch, Text_Len, D_Model]

        # Добавляем стиль спикера к выходу энкодера (memory)
        memory = self.speaker_integrator(memory, speaker_emb)

        # --- DECODER TRAINING ---
        if mel_target is not None:
            # Teacher Forcing: подаем правильный мел (сдвинутый на 1 шаг вправо)
            # Вход: <SOS> + frames[0:-1]
            # Цель: frames

            # Создаем SOS (Start of Sequence) токен (нулевой кадр)
            sos_frame = torch.zeros(mel_target.size(0), 1, Config.n_mels, device=mel_target.device)
            dec_input = torch.cat([sos_frame, mel_target[:, :-1, :]], dim=1)

            dec_input = self.mel_prenet(dec_input)

            # Добавляем PosEncoding к декодеру тоже
            dec_len = dec_input.size(1)
            dec_input = dec_input + self.pos_encoding[:, :dec_len, :].to(dec_input.device)

            # Causal Mask (чтобы декодер не видел будущее)
            ...
            return mel_out  # [Batch, Mel_Len, 80]

        return None  # Для инференса нужен отдельный метод
    ...
    @torch.no_grad()
    def inference(self, text, speaker_emb, max_len=500):
        # 1. Encode
        x = self.embedding(text)
        ...
        # 2. Decode Loop
        # Начинаем с пустого кадра
        curr_mel = torch.zeros(1, 1, Config.n_mels, device=text.device)
        generated_mels = []

        for i in range(max_len):
            # Прогоняем через PreNet и Decoder
            # В реальном Transformer надо подавать всю последовательность, накопленную ранее
            # Но для простоты (и если используем KV-cache, которого тут нет)
            # Мы просто подаем накопленный контекст.

            dec_input = self.mel_prenet(curr_mel)
            dec_input = dec_input + self.pos_encoding[:, :dec_input.size(1), :].to(dec_input.device)

            tgt_mask = self.generate_square_subsequent_mask(dec_input.size(1)).to(dec_input.device)

            output = self.decoder(tgt=dec_input, memory=memory, tgt_mask=tgt_mask)

            # Берем последний кадр
            last_frame = self.mel_proj(output[:, -1:, :])
            generated_mels.append(last_frame)

            # Обновляем вход для следующего шага
            curr_mel = torch.cat([curr_mel, last_frame], dim=1)

            # (Опционально) Stop Token condition можно добавить здесь
