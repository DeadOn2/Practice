...
def collate_fn(batch):
    ...
    return {
        "text": text_padded,
        "mel": mels_padded,
        "audio_paths": audio_paths  # Теперь в батче есть список путей
    }
...
class StudentModel(nn.Module):
    def __init__(self, vocab_size, speaker_dim=Config.encoder_dim):
        ...
        # 3. Duration Predictor (Предсказатель длительности)
        # Он учится предсказывать, сколько кадров мела приходится на 1 токен
        ...
        # 4. Проекция в мел-спектрограмму
    ...
    def forward(self, text, speaker_emb, mel_target=None):
        ...
        x = self.encoder(x)
        x = self.speaker_integrator(x, speaker_emb)  # [B, T_text, D]

        # Предсказываем длительность для каждого токена
        # В логах это будет маленькое число, например 5.4 кадра на букву
        log_durations = self.duration_predictor(x).squeeze(-1)

        if mel_target is not None:
            # --- TRAINING MODE ---
            # Нам нужно как-то сопоставить текст и аудио.
            # Самый простой способ без MAS — это просто усредненное растягивание
            # под реальную длину таргета (target_len / text_len)
            target_len = mel_target.size(1)
            text_len = x.size(1)

            # Растягиваем эмбеддинги до длины мела
            # Интерполяция — самый быстрый способ для "дистилляции"
            x_aligned = F.interpolate(x.transpose(1, 2), size=target_len, mode='nearest').transpose(1, 2)

            mel_out = self.mel_proj(x_aligned)
            return mel_out, log_durations
        else:
            # --- INFERENCE MODE (Zero-Shot) ---
            # 1. Используем предсказанные длительности
            # Умножаем на коэффициент (например, 8.0), чтобы получить кол-во кадров
            dur_rounded = torch.clamp(torch.round(log_durations * 8.0), min=1).long()

            # 2. Растягиваем последовательность (Length Regulator)
            # Мы повторяем вектор каждого токена столько раз, сколько предсказал duration_predictor
            # Это ключевой момент: теперь аудио будет длиться секунды, а не миллисекунды!
            chunks = []
            for i in range(x.size(0)):  # По батчу
                repeated = torch.repeat_interleave(x[i], dur_rounded[i], dim=0)
                chunks.append(repeated)

            # Паддим или обрезаем до макс длины в батче
            x_expanded = torch.nn.utils.rnn.pad_sequence(chunks, batch_first=True)

            mel_out = self.mel_proj(x_expanded)
            return mel_out
    ...
    try:
        for epoch in range(Config.epochs):
            ...
            for batch in dataloader:
                ...
                mel_pred, pred_durations = student(text, speaker_emb, mel_target=mels)
                ...
                loss_mel = criterion(mel_pred, mels)

                # 2. Лосс за длительность (Duration Loss)
                # Мы хотим, чтобы сумма предсказанных длительностей была близка к реальной длине аудио
                target_durations = torch.full_like(pred_durations, mels.size(1) / text.size(1))
                loss_dur = F.mse_loss(pred_durations, target_durations)
                ...
                if global_step % 10 == 0:
                    ...
                    writer.add_scalar("Loss/Duration_MSE", loss_dur.item(), global_step)
